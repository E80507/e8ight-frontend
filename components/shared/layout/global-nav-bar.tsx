...
const GlobalNavBar = () => {
  ...
  return (
    <>
      {/* 홈페이지 데스크톱 GNB */}
      <header
        className={`fixed z-[100] hidden w-full web:flex bg-white/[0.01] backdrop-blur-sm`}
      >
      ...
      {/* 홈페이지 모바일 GNB */}
      <header
        className={`pointer-events-auto fixed inset-x-0 top-0 z-[100] flex w-full items-center justify-between px-4 py-3 web:hidden bg-white/[0.01] backdrop-blur-sm ${isHome ? "h-[67px]" : "h-12"}`}
      >
        {isHome ? (
