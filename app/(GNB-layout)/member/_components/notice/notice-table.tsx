...
import { handleNoticeStatusKRText } from "@/util/string";
          ...
          table.getRowModel().rows.map((row, rowIndex) => (
            ...
            >
              {row.getVisibleCells().map((cell, cellIndex) => (
                <TableCell
                  size="sm"
                  className={`cursor-pointer ${cell.column.id === "content" ? "min-w-[250px]" : ""} ${cellIndex === 0 ? "border-x border-t" : "border-r border-t"} ${rowIndex === table.getRowModel().rows.length - 1 ? "border-b" : ""}`}
