    ...
    <div>
      ...
      <div className="px-[120px] py-[80px]">
          ...
          <div className="flex items-center justify-between">
            <div className="flex flex-wrap gap-[8px] mt-[40px]">
              {['태그1', '태그2', '태그3'].map((tag, index) => (
                <div key={index} className="px-[16px] py-[8px] h-[40px] rounded-[8px] pretendard-title-m bg-[#F4F4F6]">{tag}</div>
              ))}
            </div>

            <button className="flex items-center gap-[8px] px-[16px] py-[12px] bg-[#F9FAFA] border border-[#D6D7DC] rounded-[10px]"><Share2Icon /> 공유하기</button>
          </div>
        </div>
        ...
        <div className="flex flex-col gap-[24px] mt-[40px]">
          <div className="pretendard-subtitle-l text-[#A7A9B4]">이에이트 SNS</div>
          ...
          <div className="flex gap-[40px]">
            {[
              ...
              <Link key={social.icon} href="/">
                <Image src={`/svg/icon/${social.icon}.svg`} alt={social.alt} width={24} height={24} />
              </Link>
            ))}
          </div>
        </div>
      ...
      <div className="flex justify-center items-center relative h-[360px] overflow-hidden">
        ...
        <div className="flex justify-between items-center w-[1040px]">
          <div className="flex flex-col gap-[8px]">
            ...
            <div className="relative gibson-heading-3 text-white">The Evolution of Digital Transformation</div>
          </div>

          <Button variant="outline" shape={'round'} className="relative w-[200px] h-[59px]">기술 문의</Button>
        </div>
      </div>
    </div>
  )
